...
CREATE OR REPLACE FUNCTION __setup_copy_structure() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_create_table();
  CREATE TABLE tab_create_schema.referent (id integer PRIMARY KEY);
  CREATE TABLE tab_create_schema.shapely (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    label text NOT NULL,
    -- A plain constant Postgres renders as a cast, and so reports as possibly dynamic
    quantity bigint DEFAULT 0,
    assigned text DEFAULT 'a' CHECK (assigned = ANY (ARRAY['a', 'b'])),
    ticket integer REFERENCES tab_create_schema.referent (id),
    counted serial,
    changed timestamp with time zone,
    UNIQUE (label, quantity)
  );
  COMMENT ON COLUMN tab_create_schema.shapely.label IS 'What it is called';
  PERFORM msar.set_updated_at_column('tab_create_schema.shapely'::regclass, 7::smallint, true);
  PERFORM msar.set_column_presentation(
    'tab_create_schema.shapely'::regclass::oid, 3, '{"display_width": 210}'::jsonb
  );
  PERFORM msar.add_mathesar_table(
    'tab_create_schema'::regnamespace::oid, 'likeness', null, null, null, null, null
  );
...
CREATE OR REPLACE FUNCTION test_copy_table_structure_columns() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_copy_structure();
  RETURN NEXT columns_are(
    'tab_create_schema'::name, 'likeness'::name,
    ARRAY['id', 'label', 'quantity', 'assigned', 'ticket', 'counted', 'changed']::name[],
    'the copy has the source''s columns, and the primary key it was made with'
  );
  RETURN NEXT col_is_pk(
    'tab_create_schema', 'likeness', 'id',
    'and its own id is the key, the source''s having been left behind'
  );
  RETURN NEXT col_not_null(
    'tab_create_schema'::name, 'likeness'::name, 'label'::name,
    'a column that must have a value still must'
  );
  RETURN NEXT col_default_is(
    'tab_create_schema'::name, 'likeness'::name, 'quantity'::name, 0::bigint,
    'and one with a plain default still has it'
  );
  RETURN NEXT is(
    pg_catalog.col_description('tab_create_schema.likeness'::regclass, 2),
    'What it is called',
    'a column keeps what was said about it'
  );
  -- It would draw from the source table's sequence rather than one of the copy's own.
  RETURN NEXT is(
    (SELECT pg_catalog.pg_get_expr(adbin, adrelid) FROM pg_catalog.pg_attrdef
     WHERE adrelid = 'tab_create_schema.likeness'::regclass AND adnum = 6),
    NULL,
    'but a default drawn from a sequence is left behind'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_copy_table_structure_constraints() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_copy_structure();
  RETURN NEXT results_eq(
...
CREATE OR REPLACE FUNCTION test_copy_table_structure_triggers_and_presentation()
RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_copy_structure();
  RETURN NEXT isnt_empty(
    $q$SELECT msar.get_updated_at_triggers('tab_create_schema.likeness'::regclass, 7::smallint)$q$,
    'a column its records'' change time was kept in still has a trigger keeping it'
  );
  RETURN NEXT is(
    msar.column_presentation('tab_create_schema.likeness'::regclass::oid) -> '3' -> 'display_width',
    '210'::jsonb,
    'and how a column is shown is copied along with it'
  );
END;
$f$ LANGUAGE plpgsql;
...
BEGIN
  CREATE TYPE col_variety_enum AS ENUM ('x', 'y');
...
BEGIN
  ...
  -- Column 8
  RETURN NEXT is(
    (col_info -> 7) - ARRAY['current_role_priv'],
    $j${
  ...
  RETURN NEXT ok(
    jsonb_array_length(col_info -> 7 -> 'current_role_priv') > 0,
    'current_role_priv should be a non-empty jsonb array'
  );
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION __setup_formula_columns() RETURNS void AS $$
BEGIN
  CREATE TABLE formula_orders (
    id integer PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,
    item text,
    quantity integer,
    price numeric(8,2)
  );
  INSERT INTO formula_orders (item, quantity, price) VALUES
    ('Widget', 3, 4.50),
    ('Gadget', 10, 12.00),
    (null, 1, null);
END;
...
CREATE OR REPLACE FUNCTION __formula_attnum(col_name text) RETURNS smallint AS $$
-- plpgsql so that the table is looked for when this runs rather than when it is defined, the
-- table being one each test makes for itself.
BEGIN
  RETURN attnum FROM pg_catalog.pg_attribute
  WHERE attrelid = 'formula_orders'::regclass AND attname = col_name AND NOT attisdropped;
END;
$$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_column_is_worked_out() RETURNS SETOF TEXT AS $f$
DECLARE
  col_id smallint;
BEGIN
  PERFORM __setup_formula_columns();
  col_id := msar.add_formula_column(
    'formula_orders'::regclass,
    'total',
    jsonb_build_object('op', '*', 'of', jsonb_build_array(
      jsonb_build_object('column', __formula_attnum('quantity')),
      jsonb_build_object('column', __formula_attnum('price'))
    ))
  );
  RETURN NEXT results_eq(
    $q$SELECT total FROM formula_orders ORDER BY id$q$,
    $v$VALUES (13.50::numeric), (120.00::numeric), (null::numeric)$v$,
    'the column says what the formula works out for each record, and nothing where it cannot'
  );
  RETURN NEXT is(
    (SELECT attgenerated FROM pg_catalog.pg_attribute
     WHERE attrelid = 'formula_orders'::regclass AND attnum = col_id),
    's'::"char",
    'and Postgres is the one working it out, the column being a generated one'
  );
  RETURN NEXT is(
    (SELECT format_type(atttypid, atttypmod) FROM pg_catalog.pg_attribute
     WHERE attrelid = 'formula_orders'::regclass AND attnum = col_id),
    'numeric',
    'held as the type the formula comes to, nobody having been asked what that is'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_column_type_can_be_given() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  PERFORM msar.add_formula_column(
    'formula_orders'::regclass,
    'total',
    jsonb_build_object('op', '*', 'of', jsonb_build_array(
      jsonb_build_object('column', __formula_attnum('quantity')),
    ...
    '{"name": "numeric", "options": {"precision": 12, "scale": 4}}'::jsonb
  );
  RETURN NEXT col_type_is(
    'formula_orders'::name, 'total'::name, 'numeric(12,4)'::text,
    'a type given is the type used'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_of_text_and_a_function() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  PERFORM msar.add_formula_column(
    'formula_orders'::regclass,
    'label',
    jsonb_build_object('op', '||', 'of', jsonb_build_array(
      jsonb_build_object('fn', 'upper', 'of', jsonb_build_array(
        jsonb_build_object('column', __formula_attnum('item'))
      )),
      jsonb_build_object('value', ' x'),
      jsonb_build_object('fn', 'round', 'of', jsonb_build_array(
        jsonb_build_object('column', __formula_attnum('price'))
      ))
    ))
  );
  RETURN NEXT results_eq(
    $q$SELECT label FROM formula_orders ORDER BY id$q$,
    $v$VALUES ('WIDGET x5'), ('GADGET x12'), (null)$v$,
    'text is joined and functions are applied, and one missing value loses the whole answer'
  );
  RETURN NEXT col_type_is(
    'formula_orders'::name, 'label'::name, 'text'::text,
    'and joined text comes to text'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_making_a_choice() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  PERFORM msar.add_formula_column(
    'formula_orders'::regclass,
    'bulk',
    jsonb_build_object(
      'if', jsonb_build_object('op', '>=', 'of', jsonb_build_array(
        jsonb_build_object('column', __formula_attnum('quantity')),
        jsonb_build_object('value', 10)
      )),
      'then', jsonb_build_object('value', 'bulk'),
      'else', jsonb_build_object('value', 'single')
    )
  );
  RETURN NEXT results_eq(
    $q$SELECT bulk FROM formula_orders ORDER BY id$q$,
    $v$VALUES ('single'), ('bulk'), ('single')$v$,
    'a formula can choose between two answers'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_follows_a_renamed_column() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  PERFORM msar.add_formula_column(
    'formula_orders'::regclass,
    ...
    jsonb_build_object('op', '*', 'of', jsonb_build_array(
      jsonb_build_object('column', __formula_attnum('quantity')),
      jsonb_build_object('value', 2)
    ))
  );
  ALTER TABLE formula_orders RENAME COLUMN quantity TO how_many;
  RETURN NEXT results_eq(
    $q$SELECT doubled FROM formula_orders ORDER BY id$q$,
    $v$VALUES (6), (20), (2)$v$,
    'renaming a column the formula reads leaves every record saying what it said'
  );
  RETURN NEXT matches(
    (SELECT msar.build_formula_sql(
      'formula_orders'::regclass, msar.column_formula('formula_orders'::regclass, __formula_attnum('doubled'))
    )),
    'how_many',
    'and the formula is about the column rather than the name, so it reads the new one'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_kept_as_it_was_asked_for() RETURNS SETOF TEXT AS $f$
DECLARE
  asked jsonb;
  col_id smallint;
BEGIN
  PERFORM __setup_formula_columns();
  asked := jsonb_build_object('fn', 'round', 'of', jsonb_build_array(
    jsonb_build_object('op', '*', 'of', jsonb_build_array(
      jsonb_build_object('column', __formula_attnum('price')),
      jsonb_build_object('value', 1.15)
    )),
    jsonb_build_object('value', 2)
  ));
  col_id := msar.add_formula_column('formula_orders'::regclass, 'with_tax', asked);
  RETURN NEXT is(
    msar.column_formula('formula_orders'::regclass, col_id), asked,
    'the formula comes back as it was written, so that it can be shown and changed as it was'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_read_by_name_after_a_restore() RETURNS SETOF TEXT AS $f$
DECLARE
  asked jsonb;
  col_id smallint;
BEGIN
  PERFORM __setup_formula_columns();
  asked := jsonb_build_object('op', '*', 'of', jsonb_build_array(
    jsonb_build_object('column', __formula_attnum('quantity')),
    jsonb_build_object('column', __formula_attnum('price'))
  ));
  col_id := msar.add_formula_column('formula_orders'::regclass, 'total', asked);
  -- What a restore leaves behind: the row is against an OID that is no longer the table's, so the
  -- attnums in it mean nothing and the names are what is left to go on.
  UPDATE presentation_schema.columns SET written_against = 0
  WHERE "table" = 'formula_orders'::regclass AND attnum = col_id;
  RETURN NEXT is(
    msar.column_formula('formula_orders'::regclass, col_id), asked,
    'the formula is read back through the column names it was kept under'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_refuses_what_is_not_on_the_list() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  RETURN NEXT throws_ok(
    format(
      $q$SELECT msar.build_formula_sql(%s, '{"op": "; DROP TABLE formula_orders; --", "of": [{"value": 1}]}')$q$,
      'formula_orders'::regclass::oid
    ),
    '22023',
    null,
    'an operator that is not one of the operators is refused rather than written out'
  );
  RETURN NEXT throws_ok(
    format(
      $q$SELECT msar.build_formula_sql(%s, '{"fn": "pg_sleep", "of": [{"value": 1}]}')$q$,
      'formula_orders'::regclass::oid
    ),
    '22023',
    null,
    'and so is a function that is not one of the functions'
  );
  RETURN NEXT throws_ok(
    format(
      $q$SELECT msar.build_formula_sql(%s, '{"column": 987}')$q$,
      'formula_orders'::regclass::oid
    ),
    '42703',
    null,
    'and a column the table has not got'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_writes_a_value_as_a_value() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  PERFORM msar.add_formula_column(
    'formula_orders'::regclass,
    'note',
    jsonb_build_object('op', '||', 'of', jsonb_build_array(
      jsonb_build_object('value', $$O'Brien'); DROP TABLE formula_orders; --$$),
      jsonb_build_object('column', __formula_attnum('item'))
    ))
  );
  RETURN NEXT has_table(
    'formula_orders'::name,
    'a value written into a formula is a value, whatever it has in it'
  );
  RETURN NEXT is(
    (SELECT note FROM formula_orders WHERE item = 'Widget'),
    $$O'Brien'); DROP TABLE formula_orders; --Widget$$,
    'and it comes back out of the column exactly as it went in'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_calls_postgres_own_function() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  RETURN NEXT matches(
    msar.build_formula_sql(
      'formula_orders'::regclass,
      '{"fn": "round", "of": [{"value": 1.5}]}'::jsonb
    ),
    'pg_catalog\.round',
    'a function is called by where it lives, so one of somebody''s own cannot stand in for it'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_is_not_a_presentation_option() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  RETURN NEXT throws_ok(
    format(
      $q$SELECT msar.set_column_presentation(%s, %s, '{"formula": {"value": 1}}')$q$,
      'formula_orders'::regclass::oid, __formula_attnum('price')
    ),
    'P0001',
    null,
    'a formula cannot be set as though it were one of the ways a column is shown'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_changed() RETURNS SETOF TEXT AS $f$
DECLARE
  col_id smallint;
BEGIN
  PERFORM __setup_formula_columns();
  ...
  IF current_setting('server_version_num')::integer < 170000 THEN
    RETURN NEXT skip(
      2, 'changing a formula needs PostgreSQL 17, which is where SET EXPRESSION arrived'
    );
    RETURN;
  END IF;
  PERFORM msar.set_column_formula(
    'formula_orders'::regclass,
    col_id,
    jsonb_build_object('op', '*', 'of', jsonb_build_array(
      jsonb_build_object('column', __formula_attnum('quantity')),
  ...
  RETURN NEXT results_eq(
    $q$SELECT total FROM formula_orders ORDER BY id$q$,
    $v$VALUES (300::numeric), (1000::numeric), (100::numeric)$v$,
    'changing the formula works every record out again'
  );
  RETURN NEXT is(
    msar.column_formula('formula_orders'::regclass, col_id) -> 'of' -> 1,
    '{"value": 100}'::jsonb,
    'and the formula kept is the new one'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_refused_on_a_column_of_its_own() RETURNS SETOF TEXT AS $f$
BEGIN
  PERFORM __setup_formula_columns();
  RETURN NEXT throws_ok(
    format(
      $q$SELECT msar.set_column_formula(%s::oid, %s::smallint, '{"value": 1}'::jsonb)$q$,
      'formula_orders'::regclass::oid, __formula_attnum('price')
    ),
    '42611',
    null,
    'a column holding values of its own is not one to start working out'
  );
END;
$f$ LANGUAGE plpgsql;


CREATE OR REPLACE FUNCTION test_formula_told_in_the_column_description() RETURNS SETOF TEXT AS $f$
DECLARE
  described jsonb;
BEGIN
  PERFORM __setup_formula_columns();
  ...
  described := jsonb_path_query_first(
    msar.get_column_info('formula_orders'::regclass), '$[*] ? (@.name == "total")'
  );
  RETURN NEXT is(
    described -> 'formula' -> 'op', '"*"'::jsonb,
    'a column says what formula its values are worked out from'
  );
  RETURN NEXT matches(
    described ->> 'formula_sql', 'quantity',
    'and what Postgres works them out from, which is there for any generated column at all'
  );
  RETURN NEXT ok(
    NOT (
      jsonb_path_query_first(
        msar.get_column_info('formula_orders'::regclass), '$[*] ? (@.name == "price")'
      ) ? 'formula'
    ),
    'while a column holding values of its own has no formula question to answer'
  );
END;
$f$ LANGUAGE plpgsql;
