...
- **Range**: a range of values, such as from 1 to 10, as a PostgreSQL [range](https://www.postgresql.org/docs/17/rangetypes.html). Integer, Decimal, Date & Time, and Date columns can hold ranges, as `int4range` or `int8range`, `numrange`, `tsrange` or `tstzrange`, and `daterange`. Ranges are shown as PostgreSQL writes them (e.g. `[1,10)`, where the square bracket means the bound is one of the range's values and the round one means it isn't), and a cell has a **…** button that opens its bounds in the **Cell** section of the inspector. There the range is a **From** and a **To**, each a cell of the kind's own type, with whether the range **includes** each bound, and whether it **holds nothing** at all. Leaving a bound empty leaves the range unbounded that way.
...
- **Range** and **Array**: any number of ranges that don't overlap, as a PostgreSQL multirange, such as `int4multirange`. The Cell section edits them as a list of ranges, to change, remove, or add to; PostgreSQL puts them in order and merges any that meet, so what's saved can come back as fewer ranges than were given.
